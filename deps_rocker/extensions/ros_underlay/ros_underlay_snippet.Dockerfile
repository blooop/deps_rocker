...
ENV AMENT_PREFIX_PATH=/ros_underlay:$AMENT_PREFIX_PATH
ENV COLCON_PREFIX_PATH=/ros_underlay:$COLCON_PREFIX_PATH
ENV LD_LIBRARY_PATH=/ros_underlay/lib:$LD_LIBRARY_PATH
ENV PATH=/ros_underlay/bin:$PATH
ENV PYTHONPATH=/ros_underlay/lib/python3.12/site-packages:$PYTHONPATH
@[else]@
# No *.repos files found, create empty underlay directory
RUN mkdir -p /ros_underlay
